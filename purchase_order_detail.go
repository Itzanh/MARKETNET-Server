...
func (s *PurchaseOrderDetail) insertPurchaseOrderDetail(userId int32, trans *sql.Tx) (bool, int64) {
	if !s.isValid() {
		return false, 0
	}
...
func (s *PurchaseOrderDetail) deletePurchaseOrderDetail(userId int32, trans *sql.Tx) bool {
	if s.Id <= 0 {
		return false
	}
